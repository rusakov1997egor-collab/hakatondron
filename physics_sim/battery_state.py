...
class DronePhysicsSimulator:
    def __init__(self):
        ...
        self.current_angle = 0.0 # Угол поворота в градусах

    def tick(self, payload_weight, current_battery_pct, flight_mode, target_pos, current_pos, wind_vector={'x': 0, 'y': 0}, dt=0.1):
        """
        target_pos: {'x': ..., 'y': ...}
        current_pos: {'x': ..., 'y': ...}
        wind_vector: {'x': ..., 'y': ...} - Артём может передавать сюда рандомные значения
        """
        
        # 1. РАСЧЕТ УГЛА ПОВОРОТА (Yaw)
        dx = target_pos['x'] - current_pos['x']
        dy = target_pos['y'] - current_pos['y']
        
        # Используем арктангенс для поиска угла вектора направления
        ...
        # Влияние ветра на мощность: если летим против ветра, сопротивление выше
        # Эффективная скорость относительно воздуха
        ...
        # 3. ОТКЛОНЕНИЕ ОТ ТРАЕКТОРИИ (Дрейф)
        # Мы просто возвращаем Артёму "совет", на сколько его сдуло
        # Он сам прибавит это к координатам в main.py
        drift_x = wind_vector['x'] * dt * 0.5 # 0.5 - коэф. парусности
